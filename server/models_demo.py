...
class DemoDocument(db.Model, Serializer):
    ...
    # # Foreign key to DemoNumber
    # number_id = db.Column(db.Integer, db.ForeignKey("number.pk"), unique=True, nullable=False)
...
#     pk = db.Column("pk", db.Integer, primary_key=True)
#     time_created = db.Column(db.DateTime(timezone=True), server_default=func.now())
#     assembly = db.Column("assembly", db.Enum(AssemblyEnum), default=AssemblyEnum.FOA, nullable=False)

#     # # Relationship to DemoDocument
#     # document = db.relationship("DemoDocument", back_populates="number", uselist=False)
